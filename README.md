...
本程序通过模拟浏览器请求与课程平台 API 交互，实现自动登录、获取课程列表、遍历章节小节、以及自动播放视频（通过心跳上报进度）等功能。支持海启科技和英华两个平台，可通过命令行参数灵活控制刷课行为。
...
# 指定倍速刷课（0.5~3.0，默认 1.0）
haiqikeji -n <账号> -p <密码> --speed 2.0 --skip

# 英华平台（需额外依赖，必须指定 --url）
uv sync --extra yinghua
haiqikeji -n <账号> -p <密码> --platform yinghua --url https://scauzj.xxx.com

# 英华平台指定倍速
haiqikeji -n <账号> -p <密码> --platform yinghua --url https://scauzj.xxx.com --speed 2.0
```
...
```text
英华平台自动刷课工具 | 用户: <账号> | 地址: https://scauzj.xxx.com
尝试登录（第 1 次）...
验证码识别结果: abcd
登录成功
获取 API 令牌成功
...
| `--speed` | - | 播放速度倍数（0.5~3.0） | 1.0 |
| `--skip-complete` | `--skip` | 跳过已完成的小节 | False |
| `--list-incomplete` | `--list` | 仅打印未观看的课程/章节/小节，不执行刷课 | False |
| `--platform` | - | 刷课平台：`haiqikeji`（默认）或 `yinghua` | haiqikeji |
| `--url` | - | 英华平台基础地址（`--platform yinghua` 时必填） | - |
...
1. 使用 ddddocr 自动识别验证码完成登录（最多重试 3 次）。
2. 获取 API 令牌用于后续请求。
3. 通过 HTML 页面 XPath 解析获取未完成课程列表。
4. 分页拉取每门课程的学习记录，筛选未学习章节。
5. 每隔 30 秒（按倍速缩放）提交一次学时，直到视频总时长完成。
6. 提交间隔内平滑渲染进度条动画。
...
haiqikeji/
├── __init__.py          # 包版本
├── api.py               # 海启科技 API 请求函数和常量
├── session.py           # HTTP 会话配置（含公共重试策略）
├── progress.py          # 终端进度条渲染
├── utils.py             # 工具函数
...
└── yinghua/
    ├── __init__.py
    ├── api.py           # 英华平台 API 请求函数
    ├── session.py       # 英华平台会话配置（复用公共重试策略）
    └── cli.py           # 英华平台刷课业务逻辑
```
